...
| 2026-04-28 | Launchd live; first manual tick clean | `com.prospector.paper-trade-elder` loaded; manual `launchctl start` produced `once: {closed: 0, opened: 0, skipped_open: 0, open_after_tick: 0, nav: 10000.0}` — no fresh signal at the just-printed bar. OHLCV refresh hit transient HL 500s on a few 4h pulls; daemon caught as warnings and continued. Next scheduled tick at the 4h boundary. |
| 2026-04-28 | Dashboard wired with `crypto_perp` schema | Added schema-aware loaders/renderer to `prospector.dashboard` (no changes to kalshi loaders). Compare tab now overlays the elder net-P&L trajectory alongside the two PM books; per-strategy tab shows open/closed positions with elder columns (coin, direction, units, exit_reason, gross/fees/funding/net P&L) and a recent-ticks panel parsing `tick:` and `once:` lines. |
| 2026-04-29 | **Coin-name suffix bug — daemon was mute for 36h** | After 24h with 0 opens, traced to a suffix-stripping bug in `prospector.data.client._coin`: it stripped `-PERP` (hyphen) but not `_PERP` (underscore). The elder runner uses underscore-suffixed names internally (`BIGTIME_PERP`) and passes them through to `download_pair`, which propagates them to the API — Hyperliquid returned HTTP 500 on every request. The runner caught the 500 as a `WARNING` and continued, so it never crashed and the failure was silent in tick summaries. Fix: `_coin` now strips both suffixes. Manually backfilled the cohort (62/62 successful pulls); first signal fired immediately on HMSTR-PERP (SHORT entry, $200 risk). Lesson: a transient-error log line is also a system-level health signal — staleness detection on `data/ohlcv/<coin>/<tf>.parquet` mtime would have caught this on tick 2. |
| 2026-04-29 | OHLCV staleness audit added | Per-tick check classifies each cohort coin's short-TF parquet by freshness: stale (>2× short_tf old, missed refresh) vs. very-stale (>7d, likely delisted). Emits `WARNING` with the offending list only when something is off so healthy ticks stay quiet. This would have caught the suffix bug on tick 2 instead of tick 9. |
| 2026-04-29 | OMNI pruned from vol_q4 cohort (31 → 30 coins) | `OMNI_PERP` last 4h bar is `2025-09-25 08:00 UTC` — perp appears delisted from Hyperliquid; daemon log showed `incremental update from 2025-09-25T12:00:00+00:00` → `already up to date` every tick. Including dead coins doesn't bias the strategy (they generate no signals) but does add noise to logs and tick latency. Removed from `data/cohorts/vol_quintiles_2026-04-28.json`. The cohort doc remains pinned at 2026-04-28 — this is a sampling-error correction, not a regime-rebalance. |
